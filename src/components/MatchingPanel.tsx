...
import { Complex, zFromGamma, yFromZ, applySeriesReactance, applyShuntSusceptance, gammaFromZ } from '@/smith/math';
...
const COMPONENT_INFO: Record<ComponentType, { label: string; symbol: string; domain: string; direction: string; color: string }> = {
  series_L:  { label: 'Series L',  symbol: '⌇L',  domain: 'Impedance',  direction: 'CW',  color: '#16a34a' },
  series_C:  { label: 'Series C',  symbol: '⌇C',  domain: 'Impedance',  direction: 'CCW', color: '#2563eb' },
  shunt_L:   { label: 'Shunt L',   symbol: '⏚L',  domain: 'Admittance', direction: 'CCW', color: '#d97706' },
  shunt_C:   { label: 'Shunt C',   symbol: '⏚C',  domain: 'Admittance', direction: 'CW',  color: '#8b5cf6' },
};

export default function MatchingPanel({ state, onApplyStep, onUndoStep, onClearMatching }: Props) {
  ...
  const signedValue = useMemo(() => {
    switch (component) {
      case 'series_L': return value;    // +jX
      case 'series_C': return -value;   // -jX
      case 'shunt_L':  return -value;   // -jB
      case 'shunt_C':  return value;    // +jB
    }
  }, [component, value]);
  ...
  const preview = useMemo(() => {
    ...
    const zBefore = zFromGamma(gamma);
    const yBefore = yFromZ(zBefore);
    ...
    const zAfter = zFromGamma(newGamma);
    const yAfter = yFromZ(zAfter);
    const infoAfter = getPointInfo(newGamma, state.Z0);
  ...
  const valueLabel = isSeriesType ? 'Reactance X (norm)' : 'Susceptance B (norm)';
  ...
  return (
    <div className="p-3 space-y-4 text-sm">
      ...
      <div>
        <label className="block text-xs text-muted-foreground mb-1.5 font-medium uppercase tracking-wider">Component Type</label>
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground bg-muted/50 rounded-md px-2.5 py-1.5">
        <span className="inline-block w-2 h-2 rounded-full" style={{ backgroundColor: info.color }} />
      ...
      {/* Value input */}
      <div>
        <label className="block text-xs text-muted-foreground mb-0.5 font-medium">{valueLabel}</label>
      ...
      >
        {!activePoint ? 'Select a point first' : value === 0 ? 'Enter a value' : `Apply ${info.label}`}
      </button>

      {/* Live preview */}
      {preview && activePoint && (
        <div className="bg-muted/30 rounded-md p-2.5 space-y-1.5 border border-border">
          <div className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1">Preview</div>
          {isSeriesType ? (
            <>
              ...
              <div className="text-xs smith-mono text-muted-foreground">
                r = {preview.zBefore.re.toFixed(3)} (constant)
              </div>
            </>
          ) : (
            <>
          ...
          <div className="text-xs smith-mono">
            <span className="text-muted-foreground">VSWR:</span> {preview.vswr.toFixed(2)}
            <span className="text-muted-foreground ml-2">RL:</span> {isFinite(preview.returnLoss) ? preview.returnLoss.toFixed(1) : '∞'} dB
          </div>
        </div>
      )}
        ...
        <div className="border-t border-border pt-3 space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs text-muted-foreground font-medium uppercase tracking-wider">
              Matching Network ({state.matchingSteps.length} step{state.matchingSteps.length !== 1 ? 's' : ''})
            </label>
            <div className="flex gap-1">
              <button className="smith-btn-ghost text-xs px-2 py-0.5" onClick={onUndoStep}>
                ↶ Undo
              </button>
              <button className="smith-btn-ghost text-xs px-2 py-0.5 text-destructive hover:text-destructive" onClick={onClearMatching}>
                Clear
              </button>
            </div>
          </div>
            ...
            {state.matchingSteps.map((step, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="inline-block w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: stepInfo.color }} />
                  <span className="font-medium">{stepInfo.label}</span>
                  <span className="text-muted-foreground">
                    {isShunt ? 'ΔB' : 'ΔX'} = {step.value >= 0 ? '+' : ''}{step.value.toFixed(3)}
                  </span>
                </div>
              );
            })}
